...
void UltraFace::load(const std::string &model_path, int input_width, int input_length, float score_threshold_, float iou_threshold_) {
...
int UltraFace::detect(cv::Mat &img, std::vector<FaceInfo> &face_list) {
    ...
    cv::Mat inputBlob = cv::dnn::blobFromImage(in, norm_val, cv::Size(in_w, in_h), mean_vals, true);
